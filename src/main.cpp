...
int main() {
    ...
    // -----------------------------------------------------------------------
    // Build a deep tree BELOW the root/client boundary:
    //   TradingTechnique (root)
    //     -> techDivision (level 1, group)
    //          -> a1, a2                    (individuals at level 2)
    //          -> equitiesGroup (level 2, group)
    //               -> q1                   (individual at level 3)
    //               -> energySub (level 3, group)
    //                    -> e1              (individual at level 4)
    ...
    Division* equitiesGroup = new Division("EQ");   // level 2 group
    Worker* q1 = new Worker("Q1", "AAPL", 3);       // individual at level 3
    Division* energySub = new Division("XOM");      // level 3 group
    Worker* e1 = new Worker("E1", "XOM", 3);        // individual at level 4
    energySub->add(e1);
    equitiesGroup->add(q1);
    equitiesGroup->add(energySub);                  // group nested in a group

    techDivision->add(a1);
    techDivision->add(a2);
    techDivision->add(equitiesGroup);               // nested composite chain
    showTree("  initial tech division (4 levels deep)", techDivision);
